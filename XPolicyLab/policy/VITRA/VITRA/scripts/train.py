...
def experiment(variant):
    ...
    # ``wandb.init`` (created by VLAMetrics below) reads WANDB_API_KEY directly.
    # Avoid ``wandb.login(key=...)`` here because the login helper may persist a
    # long-lived credential in the account's netrc file on a shared trainer.
    ...
    vla_dataset, collator, batch_sampler = get_vla_dataset_and_collator(
        ...
        representation=variant["train_dataset"].get("representation", "wuji20"),
    )
