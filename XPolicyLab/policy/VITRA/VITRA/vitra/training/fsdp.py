...
class VLAFSDPStrategy(TrainingStrategy):
    ...
    def save_checkpoint(
        ...
        def save_with_time(state_dict, path):
            """Atomically publish a state dict and retain thread failures for the main rank."""
        ...
        with FSDP.state_dict_type(self.vla, self.fsdp_state_dict_type, self.fsdp_save_policy, self.fsdp_save_optimizer_policy):
            ...
            if overwatch.is_rank_zero():
                save_threads = [
                    threading.Thread(
                        target=save_with_time,
                        args=(model_state, checkpoint_dir / "weights.pt"),
                        name=f"checkpoint-weights-step-{global_step}",
                    ),
                    threading.Thread(
                        target=save_with_time,
                        args=(optim_state, checkpoint_dir / "optimizer.pt"),
                        name=f"checkpoint-optimizer-step-{global_step}",
                    ),
                ]
                ...
                if not save_errors:
                    ...
                    try:
                        ...
                        temporary_meta_path = checkpoint_dir / "meta.json.tmp"
                        with open(temporary_meta_path, "w") as f:
                            json.dump(meta_state, f)
                        temporary_meta_path.replace(meta_path)
                    except BaseException as exc:
                        save_errors.append((meta_path, exc))

            # A failed rank-zero writer must make every rank leave together instead
            # of stranding the non-zero ranks at a barrier.
            save_failed = torch.tensor(
                int(overwatch.is_rank_zero() and bool(save_errors)),
                dtype=torch.int32,
                device=torch.device("cuda", torch.cuda.current_device()),
            )
            dist.broadcast(save_failed, src=0)
            if save_failed.item():
                if overwatch.is_rank_zero():
                    failed_path, failed_exception = save_errors[0]
                    raise RuntimeError(f"Failed to save checkpoint file {failed_path}") from failed_exception
                raise RuntimeError("Rank zero failed to save the checkpoint")
    ...
    def run_training(
        ...
        with tqdm(
            ...
            for epoch in range(start_epoch, self.epochs):
                ...
                if epoch_save_interval and (epoch + 1) % epoch_save_interval == 0:
                    self.save_checkpoint(
