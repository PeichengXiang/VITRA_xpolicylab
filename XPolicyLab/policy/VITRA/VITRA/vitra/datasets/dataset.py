...
from vitra.datasets.spark0_dataset import RoboDatasetCore
...
class FrameDataset(Dataset):
    def __init__(self, dataset_folder, dataset_name,
        ...
        if data_type == 'human':
            # Keep the robot fine-tuning path independent of the upstream
            # human-video stack (decord/ffmpeg/utils3d). Those optional modules
            # are imported only when a human dataset is actually requested.
            ...
            if self.representation == "inspire12":
                from vitra.datasets.egovla_inspire_dataset import EgoVLAInspireDatasetCore

                dataset_core_cls = EgoVLAInspireDatasetCore
            else:
                dataset_core_cls = RoboDatasetCore
            self.episodic_dataset_core = dataset_core_cls(
                root_dir=root_dir,
                statistics_path=statistics_path,
                action_past_window_size=self.action_past_window_size,
                action_future_window_size=self.action_future_window_size,
