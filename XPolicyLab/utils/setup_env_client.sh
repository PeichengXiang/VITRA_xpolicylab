...
yaml_python="${EGOVLA_EVAL_PYTHON:-python}"
if ! command -v "${yaml_python}" >/dev/null 2>&1 && [[ ! -x "${yaml_python}" ]]; then
    echo "setup_env_client.sh: YAML parser Python is not executable: ${yaml_python}" >&2
    exit 2
fi
IFS=$'\t' read -r eval_batch yaml_protocol < <(
    XPOLICY_DEPLOY_YAML="${yaml_file}" "${yaml_python}" - <<'PY'
import os

import yaml
with open(os.environ["XPOLICY_DEPLOY_YAML"], "r", encoding="utf-8") as f:
